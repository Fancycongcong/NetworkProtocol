...
void connect_serv(char *serv_ip, int serv_port)
{
        ...
        if(client_socket<0)
            cse4589_print_and_log("fail to create socket\n");
        ...
        if( CONNEC < 0)
            cse4589_print_and_log("connect fail\n");
        ...
        if(!(strcmp(argv[1], "s"))) //run as server
        {
            ...
            while(1){
                ...
                if(select(fdmax+1, &read_fds, NULL, NULL,NULL)<0){
                    cse4589_print_and_log("select fail\n");
                    exit(1);
                }
                // else{
                ...
                if(FD_ISSET(i, &read_fds)){
                    ...
                    if( i == STDIN){//deal with cmd
                        ...
                        if(newfd == -1){
                            cse4589_print_and_log("accept fail\n");
                        }else{
                        ...
                        if(recv(i, buf, BUFSIZE,0)<=0){
                            ...
                            client[i].state = 0;
                            close(i);
                            cse4589_print_and_log("Client Host terminate connection\n");
                            FD_CLR(i, &master);
                        }else{
                            //processing income data from existing clients
                            cse4589_print_and_log("client send me:%s\n", buf);  
                            ...
                            if(!(strcmp(client_cmd[0], "PORT"))){
                                ...
                                // strcat(buf_list, client[i].hostname);
                                // strcat(buf_list, " ");
                                // strcat(buf_list, client[i].ip_addr);
                                // strcat(buf_list, " ");
                                // strcat(buf_list, client[i].PORT_NUM);
                                // strcat(buf_list, " ");
                                // send(i, buf_list , strlen(buf_list),0);
                                
                            }
                    
                            if(!(strcmp(client_cmd[0], "LIST"))){
                                // char *buf = (char*)malloc(sizeof(char)*BUFSIZE);
                                // memset(buf, '\0', BUFSIZE);
                                cse4589_print_and_log("BUF:%s\n",buf_list);
                                if(send(i, buf_list , strlen(buf_list),0) == -1)
                                    cse4589_print_and_log("Send fail\n");
                                ...
                                for(j = 4;j<=fdmax; j++){
                                    if(FD_ISSET(j, &master)){
                                        if(j!=listener&&j!=i){
                                            ...
                                            // memcpy(buf, )
                                            if( send(j,buf,strlen(buf),0) == -1 ){
                                                cse4589_print_and_log("Broadcast fail\n");
                                                }
                                ...
                                if(!is_valid_port(atoi(cmd[2]))){
                                    ...
                                    if(!is_valid_ip(cmd[1])||!is_correct_ip(cmd[1])){
                                        ...
                                        if(!CONNEC){
                                            ...
                                            // cse4589_print_and_log("Write the port info for server:\n");
                                            // fflush(stdout);
                        ...
                        while(login == 1){
                            ...
                            if(!(strcmp(cmd[0], "LIST"))){
                                ...
                                if(recv(client_socket, client_buffer, strlen(client_buffer), 0)>=0){
                                    ...
                                    // extract_msg(client_buffer_cpy);
                                    
                                    cse4589_print_and_log("\n%-5d%-35s%-20s%-8d\n", 1, 
                                        client_cmd[0], client_cmd[1], client_cmd[2]);
                                    cse4589_print_and_log("\n%-5d%-35s%-20s%-8d\n", i, 
                                        client_cmd[3], client_cmd[4], client_cmd[5]);
                                    cse4589_print_and_log("\n%-5d%-35s%-20s%-8d\n", i, 
                                        client_cmd[6], client_cmd[7], client_cmd[8]);
                                    cse4589_print_and_log("\n%-5d%-35s%-20s%-8d\n", i, 
                                        client_cmd[9], client_cmd[10], client_cmd[11]);
                                    cse4589_print_and_log("[%s:SUCCESS]\n", cmd[0]);
                                    cse4589_print_and_log("[%s:END]\n", cmd[0]);
                                }else
                                {
                            ...
                            if(!(strcmp(cmd[0], "RECEIVED"))){
                                ...
                                if(recv(client_socket, client_buffer, BUFFER_SIZE, 0) >= 0){
                                    ...
                                    fflush(stdout);
                                    // char client_buffer_cpy[512];
                                    // memset(client_buffer_cpy, '\0', 512);
                                    // strcpy(client_buffer_cpy, client_buffer);
                                    // extract_msg(client_buffer_cpy);
                                    char client_cmd[4][200];
                                    char *segm;
                                    int msg_index = 0;
